...
## H2H SV+H 抵消 — 查對手 RP 球隊（2026-05-29 實戰補充）

情境：H2H 對手 SP 多、投手計數類別（IP/K/QS/ERA/WHIP）已輸定，只剩 SV+H（平手）+ W 可搶 → 短期 add 1-2 個 RP 衝 SV+H **類別領先**（非長持，週後還原）。

**核心：SV+H 是「比誰多」的對戰類別 — 要看相對對手的淨值，不是絕對產出。**

- **hold 等價 save**：SV+H 合併計分，1 hold = 1 save。穩定 setup man（高出場、每次球隊領先就上）是可靠來源，**不必執著 closer**；committee closer 反而某天輪不到。評估自家與對手的 SV+H 火力都要用這個鏡頭，別只數 closer。
- **同隊抵消陷阱**：若你 add 的 RP 跟「**對手已持有的 RP**」同隊，該隊贏球時兩邊同步進帳（你 +1、對手 +1）→ 互相抵消，**不產生類別領先淨值**。等於花一次 acquisition 只做 wash。
- **操作**：評估 add 對象前，**先查對手 roster 的 RP 在哪些球隊**（`python3 _tools/_trade_lookup.py <對手隊名>`，注意**不看 selected_pos 占位、認球員實際角色** — RP 格可能是真先發、P/BN 格也可能是 closer）。優先 add「**對手沒有 RP 的球隊**」的 RP（純淨增）。
- **W 連帶**：對手的 closer 若跟你某個先發 SP 同隊，該隊贏球時你 SP 拿 W、對手 closer 拿 save — **W 與對手 SV+H 正相關**（拼那場 W 會順手餵對手一個 SV+H），取捨時要意識到。

實戰例（2026-05-29 vs BUMMER）：對手 2 closer = Muñoz (SEA) + O'Brien (STL)。擬 add 的 Ferrer 在 SEA → 撞 Muñoz，SEA 小勝時抵消（淨值 ≈ 0）；改/加 King (HOU，對手無 HOU RP) 才是純淨增。現役 Kelly (TB，對手無 TB RP) 本就是淨優勢來源。O'Brien (STL) 又跟我方先發 Pallante (STL) 同隊 → Pallante 拼 W 會餵 O'Brien save。
