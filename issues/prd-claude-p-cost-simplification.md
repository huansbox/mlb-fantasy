...
- ⏳ **待處理**：S2+S3 merge master + VPS deploy（git pull + 改 `/etc/cron.d/daily-advisor`：刪舊 22:15/05:00 兩行、加上述平日/假日兩行，原子化避開 `--morning` argparse error 窗口）+ 更正 `project_claude_p_subscription` 記憶（US 13 — 本機檔案記憶目錄查無此檔，可能在 mac 機器或 cccmemory MCP，更正前需先定位 store）+ Phase 2 model 降級（Out of Scope）。
