...
**打者表格**：
```markdown
| 球員 | 位置 | G | AVG | OPS | HR | BB | RBI | R | SB | BB% | 趨勢 |
```
...
| 標記 | 含義 | 打者觸發條件 | 投手觸發條件 |
|------|------|------------|------------|
| ▲ | 超越預測 | OPS 比預測高 .050+ 或 HR pace 超預測 20%+ | ERA 比預測低 0.50+ 或 K/9 超預測 1.0+ |
| — | 符合預測 | 各項在預測值 ±10% 內 | 各項在預測值 ±10% 內 |
| ▼ | 低於預測 | OPS 比預測低 .050+ 或 AVG 比預測低 .030+ | ERA 比預測高 0.50+ 或 IP pace 低於預測 15%+ |
...
2. **比率拖累警報**：
   - 任何正選打者 AVG < .200（拖全隊 AVG 類別）
   - 任何 SP ERA > 5.50 且 WHIP > 1.60（拖比率）

### 需要觀察（標記但不立即行動）

3. **表現偏離預測**：
   - 打者 OPS 比預測低 .100+ 且已過 50 PA
