...
cd {專案根目錄} && op run --env-file=daily-advisor/.env -- python daily-advisor/yahoo_query.py fa --position CF --count 15
cd {專案根目錄} && op run --env-file=daily-advisor/.env -- python daily-advisor/yahoo_query.py fa --position SP --count 15

# 近一週表現最佳的可用球員
cd {專案根目錄} && op run --env-file=daily-advisor/.env -- python daily-advisor/yahoo_query.py fa --sort AR --sort-type lastweek --count 15

# 整體排名最高的可用球員
cd {專案根目錄} && op run --env-file=daily-advisor/.env -- python daily-advisor/yahoo_query.py fa --sort AR --count 20
```
