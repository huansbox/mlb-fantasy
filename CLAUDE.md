...
- [ ] **/emerging-batter + /emerging-batter-deep skill 落地**（2026-05-14 設計定稿）：對稱 SP 路徑 /stream-sp + /stream-sp-deep，補 batter 短期決策 gap。主軸 role change detection（不是 hot streak），14 天觀察期 + graduation 雙路徑。設計 doc 含完整 Step 結構、訊號門檻、pending file schema、20-25 個 TDD test 列表、6 條尚待決定細節、7 步驟落地清單（預估 4-6 小時 + 1-2 週觀察期）。Reuse `yahoo_query.py fa` + `savant_rolling.json` + `roster_config.json`，無新外部依賴。詳見 [`docs/emerging-batter-design.md`](docs/emerging-batter-design.md)。
