...
- [ ] **stream-sp / stream-sp-deep 優化建議 — 調查已完成，待拍板後實行**（2026-07-07 AFK 調查）：主 issue = GitHub [#409](https://github.com/huansbox/mlb-fantasy/issues/409)（Findings F1-F7 + 建議看板 + 6 個拍板點），子 issue #404-#408（S1 scan 近況欄位 / S2 opener-bulk 盲點+角色 registry / S3 deep CLI pending 自讀 / S4 文檔 hygiene / S5 對手分級機械化）。開工先讀 #409 過拍板清單；S1+S2 高優先且同動 `stream_sp_scan.py` 建議同批。拍板點 6 含既有 `issues/011` parity 的處置（建議 OBE 結案）。
