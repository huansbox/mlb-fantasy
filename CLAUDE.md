...
- [ ] **fa_scan 決策執行層 + 量修復改善計畫 — PRD 已定稿，待 /prd-to-issues 切片**（2026-06-13）：研究（[`docs/fa-scan-eval-brainstorm-7x7.md`](docs/fa-scan-eval-brainstorm-7x7.md) 腦力激盪 + [`docs/fa-scan-decision-retrospective-2026h1.md`](docs/fa-scan-decision-retrospective-2026h1.md) 全季回溯）→ grill-me 九題對齊 + agent 模組審查 → PRD 定稿。**主 issue = GitHub [#316](https://github.com/huansbox/mlb-fantasy/issues/316)（進度看板以它為準）**，規格快照 [`issues/prd-decision-execution.md`](issues/prd-decision-execution.md)。範圍 = 執行層（ledger / 機械星等 4★+ 通知 / 慢快軌 gate）+ 量修復（platoon / SP 場次投影 / swap vector）+ 五小欄位；breakout 家族與 research_more 明列 out of scope。**開工前提 = 先收掉既有 037**；切片草案 12 片（11 AFK / 1 HITL），關鍵路徑 037 → L-a ledger → star → L-b gate。
