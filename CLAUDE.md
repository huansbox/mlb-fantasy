...
- [ ] Bug: VPS git push 403（GitHub auth 過期？詳見 `.claude/bugs/vps_git_push_403.md`）
- [x] Bug: `yahoo_query.py` savant type detection — 查兩邊 CSV 比較 BBE 取高的
- [x] Bug: falsy-zero 全掃（`main.py` / `weekly_scan.py` / `yahoo_query.py` / `roster_stats.py`）
...
- [ ] **weekly_review.py cron 提早到 TW 12:30**（UTC 04:30）：目前 TW 18:00 太晚，FAAB TW 15:00 已結算。提早後可趕同日 FAAB
- [ ] **weekly_review.py 改用 Yahoo API `roster;date={date}` 取陣容**：不依賴 roster_config.json（可能是舊的），改用 `/team/{key}/roster;date={week_start_date}` 拉指定日期的 roster。TW 12:30（= ET 00:30）拉 ET 當日 roster 已包含前一天 FAAB 處理結果，不需要等 TW 15:00
- [ ] **weekly_scan.py cron 配合提早**：目前 TW 19:30 → 改到 TW 13:00（prepare 之後）
- [ ] **weekly_review.py 守位覆蓋邏輯**：目前抓 roster 快照假設整週固定。問題：臨時填補球員（如 Hicks 只補 1 天 C 空洞）被當成整週主力。需改為用 Yahoo API `/team/{key}/roster;date={date}` 拉每日 selected_position
- [ ] **IL / NA 狀態區分**：目前 `fetch_opponent_roster()` 和守位覆蓋邏輯沒有區分 IL / NA / Active。NA 球員不佔 active roster 但被算進分析（如對手 Eldridge NA 被算成打者、我方 Whitlock NA 被算成可用 RP）。需要在 roster 分析中排除 NA 球員
