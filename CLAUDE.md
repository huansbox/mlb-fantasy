...
- [ ] **fa_scan 評斷框架升級腦力激盪 — 研究完成，待用戶裁決**（2026-06-12，multi-agent workflow 產出）：針對「breakout 偵測 + murky middle 鑑別」兩大難題，45 提案合併為 36 項評估（9 adopt_now / 18 pilot / 8 research_more / 1 reject）。核心診斷 = 現行指標全為 outcome-lagging + 無類別邊際價值概念 + 量（PA/starts）盲區 + 系統失憶。**5 個裁決點待用戶決定**（projections 哲學變更 / payload 上限 / news-miner credit pool / 週戰術介面 / 與 037 排序），裁決前不動 code。完整報告見 [`docs/fa-scan-eval-brainstorm-7x7.md`](docs/fa-scan-eval-brainstorm-7x7.md)。
