...
- [ ] **roster_sync watermark 漏交易（第三次）— 根因修復**（2026-06-11 發現）：6/10 13:08 UTC 用戶同時段兩筆異動，13:22 cron 只見其一就把 watermark 推到 poll 牆鐘時間，晚浮出的 +Dubón/−Steer 永久跳過 → config 滯後 26h → 6/11 fa_scan 把 Steer（自己剛 drop 的）當 FA 推薦加回。config 已 `--init` 手動修復（`73e7528`）。根因 = watermark 寫 `time.time()` 而非已處理交易 max(timestamp)；與 6/2 `classify_empty_diff` 修的是不同面（那次=交易可見 roster 未反映；這次=交易不可見）。修法選項 + 防呆網（fa_scan FA 池 ∩ roster alert / 每日全量 roster 對帳班）見 [`issues/roster-sync-watermark-feed-lag.md`](issues/roster-sync-watermark-feed-lag.md)。
