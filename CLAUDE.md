...
- [x] **IP/GS 指標失準**：用 game log `gamesStarted=1` 過濾，只算先發場次 IP。修正 `weekly_scan.py` + `roster_sync.py`（2026-04-07）
...
> Layer 2 改前 73 人（8 無用 RP swingman），改後 ~63 人（7 純 RP 有 SV+H 產量）
...
- [ ] **RP SV+H ≥ 2 門檻動態化**：賽季中期（Week 8+）season SV+H 膨脹，屆時需提高門檻或改公式（如 `SV+H ≥ current_week * 0.5`）

### fa_watch 對齊調整

- [ ] **prompt_fa_watch.txt**：「Punt SV+H」描述和 CLAUDE.md 不一致，需改為「RP 維持 2 位，有更好替換應行動」
- [ ] **RP stats 來源**：fa_watch 的候選 stats 是 season（`out=stats`），不是 biweekly。目前 season ≈ biweekly 無差，賽季中期可能需改用 `/stats;type=biweekly`（同 weekly_scan）
- [ ] **RP 查詢**：fa_watch 不主動查 RP（靠 ALL top 50 間接撈）。目前定位是日常監控不需要，但如果發現 RP 候選漏撿可以加 RP 查詢
