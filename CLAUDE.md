...
- [ ] **fa_scan batter 判斷品質優化（PRD 已定稿，待 /prd-to-issues）**（2026-06-10 研究定稿）：同模型下的判斷品質研究 — 三大發現：① payload 假值/缺口直接誤導判斷（watch 球員 %owned 假 0%、FA prior 無 PA、無 platoon 訊號 — Pederson 立即取代 Arraez = -28% 週 PA 量未被處理、batter 無 wOBA−xwOBA 運氣訊號）；② **「建議結案」不在 prompt 設計內**（自發行為 → 必然接不到執行；解法 = `CLOSE\|` 結構化行）；③ **決策 backtest 回路 — SP 是空殼（parse regex 不匹配 production + outcome fetch stub，兩班 cron 全「no verdicts」）、batter 連殼都沒有**（最高槓桿，先建尺再調刀）。與 payload doc 建議 2/3/4 有 3 項重疊，PRD 階段應合併為單一改善計畫。**C1 backtest 設計已 2026-06-10 grill-me 對齊定稿（10 題：修 SP+建 batter 共用引擎 / verdict 讀 waiver-log 區塊+補 vs 欄位 / 21d 實際產出六類別無 SB 無 PA / 2-agent 強制二選一+明顯勉強標註合議 / 觀察鏡像判定 / 未執行也對帳+執行標註 / 共用週日 cron+獨立 doc）**。完整發現（A1-A6 / B1-B5 / C1 / D + 優先序 + C1 設計定稿總表）見 [`docs/fa-scan-batter-judgment-quality.md`](docs/fa-scan-batter-judgment-quality.md)。**主 issue = [`issues/prd-fa-scan-batter-quality.md`](issues/prd-fa-scan-batter-quality.md)**（已切分 `issues/027`-`037`，11 片 8 AFK/3 HITL）。**之後 session 開工先讀主 issue 的「進度看板」段**（目標 / 狀態表 / 開工順序 / handoff），完工回寫狀態。曆法長竿：028 文法擴充最先部署，+21 天資料成熟期後 batter 才有第一筆可對的帳。
