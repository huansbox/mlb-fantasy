...
| `作戰策略.md` | 選秀日作戰策略：12 順位 R1-R5 決策樹+瀑布清單+即時聊天協議 | ✅ 完成 |
...
- **最終策略以 App 排名為準**（2026-03-12 擷取，前 151 名完整校正）
...
- **原始 App 排名數據**：`yahoo-app-rank.txt`（1-55）、`yahoo-app-rank - 56.txt`（56-151）
- **數據盲點**：分析基於 Steamer 預測 + 2025 賽季數據，缺乏春訓/offseason 表現、小聯盟數據。預測系統系統性低估缺乏大聯盟成績的新秀突破型球員
- **Min IP 規則**：每週未達 65 IP → ERA + WHIP 直接判負（自動送對手 2 個類別）
