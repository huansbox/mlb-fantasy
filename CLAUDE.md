...
- [x] **IP/GS 指標失準**：用 game log `gamesStarted=1` 過濾，只算先發場次 IP。修正 `fa_scan.py` + `roster_sync.py`（2026-04-07）
...
> cron 順序：fa_scan TW 12:30 → weekly_review TW 13:00。`review.scan_summary` 讀最新 fa-scan Issue。/weekly-review skill 加 Step 4 FA 行動決策。

### ~~weekly_scan 品質優化~~ ✅ 完成 / ❌ 取消（fa_scan 重構，2026-04-07）

- [x] **分批送 Claude**：fa_scan 使用 Pass 1（挑最弱）+ Pass 2（打者/SP 分開比較）
- [ ] **或拉高門檻**：目前打者/SP 2/3 核心指標通過即可，考慮加 BBE 最低門檻（如 BBE ≥ 15）刷掉極端噪音
- [x] **RP 過濾**：改為 biweekly SV+H ≥ 2 主篩 + xERA < P50 輔助。RP 無 Savant 不跳過。SP/RP swingman 走 SP 路徑。Yahoo API 用 `/stats;type=biweekly` 取真正的近兩週 stats（2026-04-07）
- ~~**RP SV+H ≥ 2 門檻動態化**~~ ❌ 取消：biweekly 為滾動兩週窗口，全季適用，不需動態調整

### ~~fa_watch 對齊調整~~ ❌ 取消（fa_watch 已被 fa_scan 取代，2026-04-07）

### 重構

- [ ] **IL/NA 排除邏輯共用化**：目前 `weekly_review.py`、`fa_scan.py`（build_roster_for_pass1）、`daily_advisor.py`（config fallback）各自實作 IL/NA 過濾。應抽成共用函式（如 `yahoo_query.py` 的 `is_active(player)`），統一 `selected_pos in ("IL", "IL+", "NA")` 判斷邏輯，減少漏改風險
