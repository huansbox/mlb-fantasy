...
- [ ] **IL / NA 狀態區分**：roster_config.json 已有 `selected_pos` + `status` 欄位（2026-04-06），`daily_advisor.py` config fallback 已用 `selected_pos` 推算 role。剩餘：`fetch_opponent_roster()` 和守位覆蓋邏輯仍需排除 NA 球員（如對手 Eldridge NA 被算成打者、我方 Whitlock NA 被算成可用 RP）
