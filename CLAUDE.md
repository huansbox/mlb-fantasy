...
**打者 wOBA−xwOBA 運氣欄位門檻**（issue 035，對稱 SP 端 xERA−ERA 標記）：2025 全季 bip ≥ 50（n=486）分布 — |gap| P50 0.014 / P70 0.023 / P90 0.040 → **顯著門檻 |gap| ≥ 0.023（P70+，對齊 SP P70 前例）**。正 = 實際優於預期 = 運氣偏多（賣高側）；負 = buy-low 側。season 需 BBE ≥40 才顯示（崩盤小樣本抑制，仿 SP BBE gate）；14d 行（BBE ≥15）只列值不標顯著（噪音基底與全季分布不可比）。腳本：`daily-advisor/calc_woba_gap_pctiles.py`。

**打者 chase / zone-contact YoY delta**（issue 049 M-bat，選球進化/崩壞早於 BB% 的先行訊號）：Savant 選球欄位 chase = `oz_swing_percent`（越低越好）、zone-contact = `iz_contact_percent`（越高越好）— **兩者都不在現有 statcast/expected batter CSV**，需 1 個 bulk `/leaderboard/custom` CSV（前提修正，同 M-sp savant_rolling）。2025 PA≥250（n=309）level 分布：chase P50 27.9 / P90 35.6；zone-contact P50 83.1 / P90 90.4。**顯著門檻 = 2024→2025 |YoY delta| P70**（n=225，對齊 woba_gap / SP xERA P70 前例）：chase |Δ| ≥ 3.6、zone-contact |Δ| ≥ 3.1。chase Δ 負 = 少追壞球 = 選球進化；正 = 崩壞。腳本：`daily-advisor/calc_discipline_pctiles.py`，引擎 `daily-advisor/batter_discipline.py`（純函式 + tag）。
...
| `daily-advisor/stream_sp_scan.py` | `/stream-sp` skill Step 2-6 機械層（TDD，tests 見 `test_stream_sp_scan.py`）— schedule parse + Yahoo FA cross-check + v4 5-slot enrich + opener/bulk filter（GS=0 全 relief：avg IP≤3 → `opener_suspect`、>3 → `bulk_suspect`，issue #405）+ `recent_form` 近況軸（近 6 場 ERA / ER≥4 崩盤數 / IP/GS / floor hint，同 `mlb_query` deep 邏輯，issue #404）+ 對手 14d + 對手 vs SP 慣用手 OPS（`vs_hand_2026`，PA<400 自動 fallback 季全 OPS + `low_pa_fallback` flag）+ `sample_warning` 2026 樣本信心 tag（BBE<30 AND GS<6 → `"low"`；其餘 BBE≤80 OR GS≤12 → `"medium"`；BBE>80 AND GS>12 → `"none"`；v4 unavailable → null。AND-for-low，**機械層不 demote verdict**，僅供 LLM 信心校正用）+ `--pending-file` 補查模式自動 diff（issue 014，emit top-level `pending_diff` key — 4 桶 still_starting/lost_to_others/replaced/no_longer_scheduled）→ JSON。CLI: `python3 stream_sp_scan.py --et-dates YYYY-MM-DD[,YYYY-MM-DD] [--pending-file PATH]`。skill 觸發 / 非 cron。e2e ~5s |
| `daily-advisor/tests/test_stream_sp_scan.py` | stream_sp_scan 單元測試（覆蓋 classify_opener 含 bulk_suspect 邊界 / compute_recent_form 近況軸 / tier_opponent / parse_schedule / cross_check_fa / _enrich_v4 / _apply_vs_hand_gate / compute_sample_warning AND-for-low + 三軸邊界 / scan 注入 Fetchers 端到端 + vs_hand sample gate boundary + sample_warning low/medium/none/v4-unavailable + compute_pending_diff 四桶 + homonym 同名邊界 + pending_diff emission contract）|
| `daily-advisor/stream-sp-roles.md` | `/stream-sp`・`/stream-sp-deep` 角色確認 registry（issue #405）— WebSearch / deep 確認過的 opener / bulk / true_starter 結論，TTL 21 天內沿用免重付 WebSearch；GS=1 且 IP≥5 觸發提前重驗。不受 pending 過期清理影響 |
| `daily-advisor/pending_parser.py` | `/stream-sp` pending file parser（issue 014 + #406）— pure-function 解析 `stream-sp-pending.md` 的 H2 `## ET YYYY-MM-DD` section 結構，輸出 `{et_date: {tbd_games, evaluations}}`；evaluation 欄位 header-driven（新舊 schema 都收）：name/team/is_home + mlb_id/sum26/sum25/opp_abbr（舊格式缺欄 → None）。消費者：stream_sp_scan.py `--pending-file`（pending_diff）+ mlb_query.py `deep --pending-file`（players list）。Corrupt schema graceful skip（free-form 備註 / 缺欄 row / malformed markdown 不 raise）|
...
| `daily-advisor/mlb_query.py` | `/stream-sp-deep` skill 用的 MLB Stats API helper — `gamelog_with_qs(mlb_id, season)` 加 ip_decimal+qs 欄；`opponent_context(team_id, end_date, sp_id)` 一次回 7d/14d/30d 趨勢 + vs SP 慣用手 split（pitchHand 內部 resolve）；`deep_batch(players)` orchestrator（issue 016）把 N 位 SP 深評壓成 1 個 SSH 批次（loop call 既有 helpers，不重寫 fetch）+ emit 7-column `comparison_table` raw JSON 讓 deep skill §4 不再手填；game log 每場 row 帶 `opp_season_ops`/`opp_tier`（issue #408 — 全聯盟 season OPS 每 run 一拉，機械分級 強≥.770/中強/中/弱≤.680 取代 skill 記憶 take 清單）。CLI 預設 `python3 mlb_query.py deep --pending-file stream-sp-pending.md --et-date D [--names "N1\|N2"] [--pretty]`（issue #406 — mlb_id/Sum/對手全從 pending 表自讀，缺 mlb_id 整批報錯）；fallback 手抄模式 `--players ID1,ID2 --et-dates D1,D2 [--opp-teams T1,T2] [--sp-names "N1\|N2"] [--opp-abbrs A1,A2] [--sum26 S1,S2] [--sum25 S1,S2]`。Partial failure: 單 SP fetch 失敗 → 該 SP `by_player[id]={"error":...}`；`MlbIdNotFoundError` 整批 raise（避免靜默丟失）。Pure functions: parse_ip / is_quality_start 可單測。Skill 觸發 / 非 cron。 |
...
| `daily-advisor/calc_woba_gap_pctiles.py` | 打者 wOBA−xwOBA gap 2025 分布推導（issue 035 運氣欄位顯著門檻 0.023 = \|gap\| P70；bip ≥50, n=486）|
| `daily-advisor/batter_discipline.py` | 打者 chase / zone-contact YoY delta 引擎（issue 049 M-bat）— bulk Savant custom fetch + 純 compute（delta / percentile / 顯著）+ tag。stdlib only、不碰 Yahoo。2025 baseline + P70 門檻內建為常數 |
| `daily-advisor/calc_discipline_pctiles.py` | chase / zone-contact 2025 level 分布 + 2024→2025 |YoY delta| 門檻推導（issue 049；PA≥250, n=309/225）|
| `daily-advisor/prospect_pedigree.py` | 打者 post-hype 新秀標記引擎（issue 049）— load / stale 偵測 / pedigree join / post-hype tag。資料資產 `prospect_pedigree.json`（mlb_id→best_rank，每年 3 月人工刷新），builder `build_prospect_json.py`。詳見 `docs/prospect-pedigree.md` |
...
- [ ] **Yahoo API 申請決策點 — 2026-08-19 後回查**（2026-08-05 定）：Yahoo 於 2026-07-28 撤銷本 app 的 API 存取（背景見本檔頂端狀態區塊 + memory `project_yahoo_api_revoked`）。已決定**先不送新申請**，改用社群回報當判斷依據。**回查動作**：看 [`uberfastman/yfpy#84`](https://github.com/uberfastman/yfpy/issues/84) 有無人回報新制申請成功 — jdguggs10 於 2026-07-29 用 App ID 綁定既有 app 送件，8/19 滿 3 週（= RosterSage 從送件到核准的週期）。**判準**：① 有人成功 → 參考其填法再決定要不要送；② 仍無人成功 → 基本確定個人開發者已被排除，關掉這條路。**先不送的理由**：截至 2026-08-05 全網 0 個成功案例，唯一走完新制的 RosterSage（企業）拿到 credentials 用了 7 週後同樣在 7/22 被斷；而等待成本為零（本季已無望，read 早三週晚三週無差別）。**若決定要送**：申請表為 B2B 導向，14 欄中 Business Title / Business Name & Address / Company Description / Consumer-Facing Product Name 等必填欄位個人無法誠實填寫，Expected Users 下拉最小選項為「< 1,000」；App ID 欄位是社群認為唯一的既有 app re-bind 路徑，不可留空。
- [ ] **stream-sp 優化批首航被動觀察**（2026-07-07 補查首航後留）：① **registry 沿用路徑尚未實戰觸發**（首航唯一新候選被 Sum floor 先擋、未走到角色確認）— 下次 scan 遇 `opener_suspect`/`bulk_suspect` 時確認「registry 沿用（confirmed_at）免 WebSearch」生效；② **registry 盲邊**：機械判 `true_starter` 時不查 registry，role-capped SP（Alvarez 型：混 GS 且 avg IP >4）會以 true_starter 進主表、靠 pending deep verdict 兜底 — 再實戰出現一次就在 `stream-sp.md` 過濾規則加一行「主表候選也對照 registry」（已入 wiki Tech-Debt 低利息）；③ **floor cap 拍板點未經用戶明示確認**（採 #409 建議值）— 首次實戰生效時看一眼合不合用，翻案 = 刪 `stream-sp.md` 報告原則一行。
- [x] ~~**stream-sp / stream-sp-deep 優化建議（#409 + 子 issue #404-#408）**~~ → **全部完成（2026-07-07）**：S1 recent_form 近況軸 + floor cap、S2 bulk_suspect + 角色 registry（`daily-advisor/stream-sp-roles.md`，TTL 21 天）、S3 deep CLI `--pending-file` 自讀（pending 表加 mlb_id 欄）、S4 hygiene 六項（含 block-local-yahoo git 放行）、S5 opp_tier 機械分級（記憶 take 清單刪除）。拍板全採 #409 建議值（僅拍板點 1 由用戶即時確認 A 輕量版）。各 slice 驗收紀錄見 #404-#408 關閉留言；`issues/011` 以 OBE 結案（deep_batch 上線 6 週 ~20 次實戰無數值異常 + 本輪 S3 e2e 雙模式對照全等 + S5 對照官方端點，已覆蓋當時要防的 regression）。
- [ ] **fa_scan 決策執行層 + 量修復改善計畫 — 14 片全實作完成 + 三段 A/B 全通過，唯一開放 #321（暫緩）**（2026-06-13 起 → 2026-07-08 收官）：研究（[`docs/fa-scan-eval-brainstorm-7x7.md`](docs/fa-scan-eval-brainstorm-7x7.md) 腦力激盪 + [`docs/fa-scan-decision-retrospective-2026h1.md`](docs/fa-scan-decision-retrospective-2026h1.md) 全季回溯）→ grill-me 九題對齊 + 兩輪 agent 審查（prd-to-issues + write-a-prd deep-module）→ PRD 定稿 → 14 子 issue。**主 issue = GitHub [#316](https://github.com/huansbox/mlb-fantasy/issues/316)（進度看板以它為準，子 issue #317-#330）**，規格快照 [`issues/prd-decision-execution.md`](issues/prd-decision-execution.md) + `issues/038`-`051`。範圍 = 執行層（ledger / 機械星等 4★+ 通知 / 慢快軌 gate）+ 量修復（platoon / SP 場次投影 / swap vector）+ 五小欄位；breakout 家族與 research_more 明列 out of scope。**037 觸發 schema 地基已部署（merge `585a736`）**。關鍵路徑 #317 ledger → #319 star → #320 gate →（#330 KPI）；可立即平行 #317/#322/#323/#325/#328/#329；HITL 唯一 #321。開工先讀 #316 看板。**#328（049 micro-fields batter）引擎兩半全完成 merge（PR #333 post-hype + PR #334 chase/zone-contact delta）；唯一剩項 payload 注入已移交 318b 全注入批（同 #322-#327，需 payload_budget gate + 行格式 + VPS A/B）。** **318b-batter 注入已 merge（PR #351 `9449fa2`）+ VPS deploy + 段① VPS 配對 A/B 通過（同 13 候選 injection on/off：input +3% 對應注入 1063 chars、output −17% 零 thinking 誘發，2026-06-18）。042（#321）暫緩 — 事實型 chase/zone tag 已被 LLM 自發使用（不需強制），ledger 規則增益本批未證實；重評觸發 = B7 已跑完（✓ 2026-07-07）+ 出現真實翻供/drop 回溯案例。**B6 SP 注入 + B7 backfill 已完成（2026-07-07）**：B7 `backfill_ledger.py` 實跑 22 筆驗收通過（roster 23/23 add_reason / watchlist 11/11 channel）；B6 merge `7ecdfd1` — slim_entry 注入 046 next_week_starts（retro gate 85.3%，probable 錨定 + 球隊比賽日 staleness 免疫 All-Star break + per-team horizon）/ 050 micro（CSW 21d、velo delta、K-BB ladder）/ ledger 記憶 2 行 / 048 swap（4★+）+ SP channel/stars wiring，prompt 未動。**段③ SP A/B ✅ 通過（2026-07-08，runner `_tools/_ab_318b_sp_runner.py`；07-08 capture fixture 反注入配對，主 model Opus 4.8[1m]）**：input +8.0%/step（+4,490 tok/scan）、output 淨 **−152 tok**（Step A −35.5% richer-data 更果斷 / Step B +129.5% verdict 多料，兩步 thinking 重分配相抵）、每 scan 成本 **+0.5%（~$0.002）**、無 lever-2 誘發（B6 未動 prompt）。char +25% 但 token 僅 +8%（JSON ~2 chars/tok + 28K Opus base）。**318b 三段 A/B（段①batter / 段③SP）全通過 → #316 執行層 14 片實質收官，唯一開放 = #321（暫緩）。**fetcher VPS endpoint 已驗 discipline+ledger（platoon/PA 未觸發待補）。完整數據見 [`docs/318b-injection-design.md`](docs/318b-injection-design.md)「段①/段③ 實測結果」。**
- [ ] **claude -p 成本簡化（必要性砍除 pass）— S1+S2+S3 + Phase 1.5 lever 1a 已 deploy**（2026-06-06 / 1.5 於 06-08）：背景 = Anthropic 6/15 起 `claude -p` / Agent SDK 改吃獨立 $100/月 credit pool（見記憶 `project_claude_p_credit_pool`，**舊「走訂閱不計費」假設已失效**）。**已完成**：S1 退役 `--rp` 週掃（2026-06-05）+ S2 日報 2 合 1（單一 adaptive 報、去 `--morning`）+ S3 cron 平日/假日分流（平日 `30 21 * * 1-5` = TW 05:30 報 ET 夜場 / 假日 `30 14 * * 6,0` = TW 22:30 報 ET 日場；merge `51bab0c` + VPS deploy + dry-run 驗證 + import 鏈無 dangling）。**Phase 1.5（fa_scan claude -p 成本）**：lever 1a（claude -p 從無 CLAUDE.md 的 neutral cwd 跑）已 deploy（commit `9c5c517`，省固定 22.5K input/call ≈ $13/月、決策+格式零變）；lever 2（batter output 收斂）2026-06-08 真實 payload 配對 A/B 證實 backfire（「只在實質變化時 UPDATE」的 skip 規則誘發 ~12K thinking → output_tokens 6.7K→18-19K、cost +68%）**已放棄**（master prompt 未動，見記憶 `feedback_claude_p_thinking_induction`）。**剩餘**：① 首班假日早報 14:30 UTC（TW 22:30）被動觀察（Telegram + GitHub issue `[日報]`）；② mac session 刪舊錯誤記憶 `project_claude_p_subscription`（Windows 端已建正確版，mac 端無法本機存取）；③ Phase 2 model 降級（Opus→Sonnet/Haiku，Out of Scope，待觀察後另案）；④ Phase 1.5 lever 1a 首班 cron 2026-06-09 TW 12:30 被動觀察（batter/SP issue 正常 + 無 `fa-scan-error`；lever 2 放棄後原「等 1a 確認再 deploy lever 2」gate 消失）。完整脈絡見 [`issues/prd-claude-p-cost-simplification.md`](issues/prd-claude-p-cost-simplification.md)。
- [ ] **fa_scan batter payload 優化（waiver-log 歷史段瘦身）**（2026-06-10 探索定稿）：batter call ~$0.49（input 52K 中 payload 35K，**waiver-log 觀察歷史佔 payload 72.4%** 且只進不出、每日淨增 15-20 行複利成長）。真實 payload 配對 A/B 證實「歷史截斷（觸發 + [eval] + 最近 5 天）」砍 46% payload、cost −17%、核心決策不變（邊際代價：長歷史觸發紀律弱化，需機械層補 counter 摘要行）。**①結案 backlog 執行 + /weekly-review Phase 1C 納入 ✅ 2026-06-10**；②建議結案自動化 ✅ 已被 issue 028 的 CLOSE 結構化行取代（LLM 發 CLOSE → 寫入端即時搬已結案，免「連 N 天偵測」）；③payload 讀取端歷史截斷 ✅ 2026-06-10（issue 032，merge `d18207e`，觀察中段 −59.7% + [機械計數] counter 摘要行）；④hygiene 小修 ✅ 2026-06-10（issue 033，merge `fc55fae`）；⑤model 降級 batter 先試（歸 Phase 2）。①-④ 全結，僅剩 ⑤ 待對帳基線（PRD C1）建立後另案。完整量測 + A/B 證據 + 決策狀態見 [`docs/fa-scan-batter-payload-optimization.md`](docs/fa-scan-batter-payload-optimization.md)。
- [x] ~~**fa_scan batter 判斷品質 + C1 對帳回路（027-037，11 片）**~~ → **全數 merge + C1 production 驗證完成（2026-07-08）**：11 片 8 AFK/3 HITL 全 ✅。曆法長竿到期後兩份對帳都產出真帳 — **SP** 自 2026-06-21（027 空殼三破洞修復生效）、**batter** 自 2026-07-05（17 episode，裁判合議 2 calls，031 executed + 051 KPI 全真值）；**030 裁判抽查 2026-07-08 PASS**（magnitude override / 雙勉強→難分 / 平局破切 / watch 看走眼四路徑正確，prompt 無需修）。「尺已建好且驗證會量」。進度 / 狀態以主 issue [`issues/prd-fa-scan-batter-quality.md`](issues/prd-fa-scan-batter-quality.md) 進度看板 + `docs/batter-decisions-backtest.md` / `docs/sp-decisions-backtest.md` 為準。研究底稿 `docs/fa-scan-batter-judgment-quality.md`。**剩被動**：037 觸發 schema 一週 spot-check（06-13 部署起）。**下游解鎖**（皆需對帳基線再累積 2-4 週後另案 A/B）：model 降級（Phase 2）/ 訊號擴充有效性 / xwOBACON Use Case B。
- [x] ~~**roster_sync watermark 漏交易（第三次）— 根因已修，待首班 cron 驗證**~~ → **驗證完成（2026-06-12）**：首班 reconcile cron（TW 11:50）log 確認「Reconcile: config matches Yahoo roster. Done.」，端到端走通，issue 關閉。修復內容：`compute_watermark`（max seen tx ts，monotonic）+ `--reconcile` 每日全量 roster 對帳防呆網（VPS cron TW 11:50）。詳見 [`issues/roster-sync-watermark-feed-lag.md`](issues/roster-sync-watermark-feed-lag.md)。
- [x] ~~**Roster freshness pipeline — 確認 6/3 次日生效 claim 自動同步**~~ → 驗證完成：Buehler/McDonald 已自動同步進 config（後續正常運作至 6/10 watermark 案，見上條）。原文（2026-06-02 修復後被動觀察）：`classify_empty_diff` 的 advance_alert 分支 2026-06-02 在 prod 首次觸發（Telegram 警告），但根因**不是** read-after-write lag，而是先前沒考慮到的 **Daily-Tomorrow 次日生效 claim**：Walker Buehler waiver claim（drop Trevor McDonald）於 6/2 07:12 UTC 以當天時間戳記錄為 successful，但 roster 效果到 ET 6/3 才反映（`fetch_full_roster` 預設查 today-in-ET，swap 在日期翻轉前對 diff 隱形）。原 `MAX_ROSTER_LAG_SECONDS=2h` 在次日生效前就放棄、推進浮水印越過該交易 → 會永久漏掉。**已修**（commit `1a56c6f`）：窗口拉到 30h（次日生效 + slack），retry path 靜默重試到 ET 日期翻轉；已回退 VPS `.last_sync` 到 1780384367 重新曝光該交易，dry-run 確認走 retry 不 advance_alert。**剩餘被動觀察**：6/3 04:07 UTC（= 12:07 TW）首班 cron 後，確認 config 自動 +Buehler / -McDonald + git auto-commit + Telegram 通知（查 `/var/log/roster-sync.log` 或 git log）。若沒同步，查 log。
...
- [x] ~~**B2 backtest cron 首跑驗證**~~ → **驗證完成（2026-06-10），結果 = cron 機制正常但對帳功能是空殼**：5/31、6/7 兩段都自動 append + commit 成功（排程/git 鏈 OK），但兩段皆「no verdicts」— 根因 ① `_STEP_B_BLOCK_RE` 不匹配 production issue 格式（Step B JSON 被 ` ``` ` fence + `</details>` 包住，regex 要求後接 `===` 或字串結尾；拿 #305 真實 body 實測 False；單元測試全綠因為用手寫假樣本）② `fetch_post_verdict_xwobacon` 是 stub（`return None` → 全 neutral）。**修復併入 fa_scan batter 判斷品質 PRD 的 C1**（範圍定案：修 SP + 建 batter 共用引擎），見 [`docs/fa-scan-batter-judgment-quality.md`](docs/fa-scan-batter-judgment-quality.md) C1 段。**空殼已由 027 修復並 production 驗證閉環（2026-07-08）**：SP regular 段自 2026-06-21 產出真 verdict（06-21/06-28/07-05 = 0%/40%/100%），詳見上方 batter 判斷品質完成項。
