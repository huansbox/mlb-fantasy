...
> 設計依據：`docs/sp-framework-v4-balanced.md`。Production live = v4 + Phase 6 multi-agent（`fa_scan.py` default `SP_FRAMEWORK_VERSION=v4`）。v2 已退役（2026-04-28 cutover），rollback 經 `SP_FRAMEWORK_VERSION=v2` 環境變數啟用；v2 用的 xERA / xwOBA allowed / HH% allowed 百分位請參考下方 RP 百分位表（指標分布完全重疊）。
...
注意：RP 品質指標（xERA/xwOBA/HH%/Barrel%）採用本表（已退役 v2 SP 共用同樣聯盟分布），K/9 和 IP/Team_G 為 RP 專用。
