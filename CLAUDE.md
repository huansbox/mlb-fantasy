...
- [ ] **claude -p 成本簡化（必要性砍除 pass）— S1+S2+S3 已 deploy**（2026-06-06 落地）：背景 = Anthropic 6/15 起 `claude -p` / Agent SDK 改吃獨立 $100/月 credit pool（見記憶 `project_claude_p_credit_pool`，**舊「走訂閱不計費」假設已失效**）。**已完成**：S1 退役 `--rp` 週掃（2026-06-05）+ S2 日報 2 合 1（單一 adaptive 報、去 `--morning`）+ S3 cron 平日/假日分流（平日 `30 21 * * 1-5` = TW 05:30 報 ET 夜場 / 假日 `30 14 * * 6,0` = TW 22:30 報 ET 日場；merge `51bab0c` + VPS deploy + dry-run 驗證 + import 鏈無 dangling）。**剩餘**：① 首班假日早報 14:30 UTC（TW 22:30）被動觀察（Telegram + GitHub issue `[日報]`）；② mac session 刪舊錯誤記憶 `project_claude_p_subscription`（Windows 端已建正確版，mac 端無法本機存取）；③ Phase 2 model 降級（Opus→Sonnet/Haiku，Out of Scope，待觀察後另案）。完整脈絡見 [`issues/prd-claude-p-cost-simplification.md`](issues/prd-claude-p-cost-simplification.md)。
