...
- [ ] **roster_sync watermark 漏交易（第三次）— 根因已修，待首班 cron 驗證**（2026-06-11 發現 / 06-12 修復部署 merge `53332fb`）：根因 = watermark 寫 poll 牆鐘時間而非已處理交易 max(timestamp)，Yahoo feed 晚浮出的同批交易永久跳過（6/10 +Dubón/−Steer 案，config 滯後 26h → fa_scan 把自己剛 drop 的人當 FA 推薦加回）。已修：`compute_watermark`（max seen tx ts，monotonic）+ `--reconcile` 每日全量 roster 對帳防呆網（VPS cron TW 11:50，diff → 自動修復 + commit 標 reconcile recovery + Telegram 警報）。**剩餘**：首班 reconcile cron（06-12 TW 11:50）後查 `/var/log/roster-sync.log` 確認「config matches」行；部署當下 Yahoo API 整體故障（500/502/503），e2e dry-run 未跑成，cron 恢復後自然驗證。詳見 [`issues/roster-sync-watermark-feed-lag.md`](issues/roster-sync-watermark-feed-lag.md)。
