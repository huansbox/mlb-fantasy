...
| `docs/handoff-yahoo-names-filter.md` | Yahoo `--names` filter CLI 層 handoff（2026-05-13）— function 層短路已完成（commit `b672713`），CLI 層 + importable `query_fa()` helper 待辦 |
...
- [ ] **Yahoo `--names` filter CLI 層**（2026-05-13 部分完成）：function 層短路已上線（commit `b672713`，e2e 5.2s），但 `yahoo_query.py fa` CLI 仍無 `--names` flag — 手動 debug 特定球員 FA 狀態還是要 grep 12 頁。下一步：CLI 加 `--names "a,b,c"` + 抽 `query_fa()` importable helper 讓 stream_sp_scan 改 import 不重複 paging 邏輯。預估 ~60 分鐘。詳見 [`docs/handoff-yahoo-names-filter.md`](docs/handoff-yahoo-names-filter.md)。
