...
- [ ] **fa_scan batter payload 優化（waiver-log 歷史段瘦身）**（2026-06-10 探索定稿）：batter call ~$0.49（input 52K 中 payload 35K，**waiver-log 觀察歷史佔 payload 72.4%** 且只進不出、每日淨增 15-20 行複利成長）。真實 payload 配對 A/B 證實「歷史截斷（觸發 + [eval] + 最近 5 天）」砍 46% payload、cost −17%、核心決策不變（邊際代價：長歷史觸發紀律弱化，需機械層補 counter 摘要行）。**①結案 backlog 執行 + /weekly-review Phase 1C 納入 ✅ 2026-06-10**；②建議結案自動化（連 3 天建議結案 → 機械移已結案，仿 auto-close rostered）⏳；③payload 讀取端歷史截斷（零 prompt 變動，無 lever 2 thinking induction 風險）⏳；④hygiene 小修（`_fmt_fa_block_batter_v4` 14d Savant BBE gate / prompt 補「%owned 為 Yahoo 全平台值」一行）⏳；⑤model 降級 batter 先試（歸 Phase 2）。完整量測 + A/B 證據 + 決策狀態見 [`docs/fa-scan-batter-payload-optimization.md`](docs/fa-scan-batter-payload-optimization.md)。
