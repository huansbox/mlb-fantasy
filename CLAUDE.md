...
> 設計依據：`docs/sp-b2-cutover-design.md`。Production live = B2 thin + 2-step single-LLM。B1 multi-agent 已退役（2025 prior + urgency + slump hold + M1/M4' metrics 全清）— rollback 走 `git revert <B2 merge commit>`，不可 reset/checkout 到 hash（會丟失 B2 開發期間其他無關 commit）。

**兩層分工**：
- **機械層（Python，`fa_compute.py` + `anchor_filter.py`）**：anchor_filter（cant_cut + weekly_anchor_sp）→ BBE<30 filter → Rotation Gate → 5-slot Sum 排序 → top-3。Sum 是內部排序鍵，**不暴露 LLM**；無 urgency / 無 slump hold / 無 Sum 硬門檻。
...
兩層機制：
- **Backtest 自動化（Use Case A，issue 024）**：週一跑 `daily-advisor/backtest_track.py` 讀過去 7 天 fa-scan SP-v4 Issues → 抽 verdict + 後續 SP 表現 → 算 hit-rate + marginal benefit → 追加進 `docs/sp-decisions-backtest.md`。慢但高訊號（週級）。
...
- [ ] **SP B2 cutover — VPS gray rollout（issue 025，HITL）**：所有 code 已 merge 到 feat 分支；待用戶人工 VPS 灰度部署 — capture B1 hash → atomic merge → stop cron → manual trigger 1-2 fa-scan → verify output → re-enable cron。詳見 `issues/025-sp-b2-vps-gray-rollout.md`。
