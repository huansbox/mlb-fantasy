...
兩層機制：
- **Backtest 自動化（Use Case A，issue 024）**：每週日跑 `daily-advisor/backtest_track.py` 讀過去 7 天 fa-scan SP-v4 Issues → 抽 verdict + 後續 SP 表現 → 算 hit-rate + marginal benefit → 追加進 `docs/sp-decisions-backtest.md`（與 `/weekly-review` 週日 cadence 對齊）。慢但高訊號（週級）。
...
- [ ] **SP B2 cutover — 24-48h 監控（2026-05-28 ~TW 15:30 production live）**：cutover 完成 — merge `95f879a` pushed、VPS pull、fa-scan cron re-enabled、backtest cron 安裝（Sun 06:00 UTC 首跑 2026-05-31，wrapper `cron_backtest.sh` commit `e55fbd5`）。首次 B2 production fa-scan = GitHub Issue #255「pass」verdict 結構正確、anchor 隱形（Skubal 不出現）、Step A/B 全綠。**剩餘**：24-48h 觀察 cron auto-run（2026-05-29 04:30 UTC = TW 12:30 為第一次自動 run）有無 unexpected behavior，平穩後關閉 issue 025。Rollback: `git revert -m 1 95f879a`。詳見 `issues/025-sp-b2-vps-gray-rollout.md`。
