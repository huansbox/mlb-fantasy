...
- [ ] **fa_scan 評斷框架升級腦力激盪 + 全季決策回溯 — 研究完成，待用戶裁決**（2026-06-12）：①腦力激盪（multi-agent workflow）：45 提案合併 36 項評估（9 adopt_now / 18 pilot / 8 research_more / 1 reject），見 [`docs/fa-scan-eval-brainstorm-7x7.md`](docs/fa-scan-eval-brainstorm-7x7.md)；②全季回溯（回答「Walker 孤例」質疑）：add 端 ~8/10 命中、**最貴失誤在 churn/drop 紀律（Hicks/Clemens/Steer）與執行洞（Vargas .945/8HR 全季最大漏接）**、heat-led 推薦幾乎全敗（Sheets/Pederson）、structure-led vs heat-led 發現路徑是最強區辨器，見 [`docs/fa-scan-decision-retrospective-2026h1.md`](docs/fa-scan-decision-retrospective-2026h1.md)。**5 個裁決點 + 修正後優先序（執行層 ≥ 量修復 > 新訊號）待用戶決定**，裁決前不動 code。
